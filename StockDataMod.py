    ...
    for filename in os.listdir(path):  # 遍历路径下所有文件
        ...
        if stockName in filename:
            if filename.count('+') == 2:  # 存在CSV文件
                ...
                if ((dtm_dfLoadTimeS - stockTimeS).days <= 0) and (
                        (dtm_dfLoadTimeE - stockTimeE).days >= 0):  # 起止日期在文件内则读取CSV文件获取数据
                    print("123", (dtm_dfLoadTimeS - stockTimeS).days)
                    print("345", (dtm_dfLoadTimeE - stockTimeE).days)
                    stockDat = pd.read_csv(os.path.join(path, filename), parse_dates=True, index_col=0,
                                           encoding='gb2312')
...
def GetStockDatPro(stockName=None, stockTimeS=None, stockTimeE=None):
    ...
    # 处理MACD
    stockPro['macd_dif'], stockPro['macd_dea'], stockPro['macd_bar'] = talib.MACD(stockPro['Close'].values,
    ...
    for day_ind in range(xd, len(date)):
        RSV[date[day_ind]] = (stockPro.Close[day_ind] - stockPro.Low[day_ind - xd:day_ind + 1].min()) / (
                    stockPro.High[day_ind - xd:day_ind + 1].max() - stockPro.Low[day_ind - xd:day_ind + 1].min()) * 100
        if day_ind > xd:
            index = day_ind - xd
