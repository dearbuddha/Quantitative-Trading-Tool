...
class MPL_Panel_Base(wx.Panel):
    def __init__(self, parent):
        ...
        self.figure = Figure()
        gs = gridspec.GridSpec(4, 1, left=0.05, bottom=0.10, right=0.96, top=0.96, wspace=None, hspace=0.1,
    ...
    def draw_subgraph(self, stockdat, numt):
        ...
        """ 绘制K线图 """
        ohlc = list(zip(np.arange(0, len(stockdat.index)), stockdat.Open, stockdat.Close, stockdat.High,
                        stockdat.Low))  # 使用zip方法生成数据列表
...
class Loop_Panel_Base(wx.Panel):
    def __init__(self, parent):
        ...
        gs = gridspec.GridSpec(3, 1, left=0.05, bottom=0.10, right=0.96, top=0.96, wspace=None, hspace=0.1,
